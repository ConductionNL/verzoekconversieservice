# verzoekconversieservice chart

The Helm chart installs verzoekconversieservice and by default the following dependencies using subcharts:
...
```bash
helm repo add verzoekconversieservice https://raw.githubusercontent.com/ConductionNL/verzoekconversieservice/master/api/helm/
helm repo update
```
...
```bash
helm install my-verzoekconversieservice ConductionNL/verzoekconversieservice --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `verzoekconversieservice` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `verzoekconversieservice` |
